...
void repartirRegalos(datosEntrada const& datos, sol& sol, std::vector<bool> & marcas, int k, int & satisMejor, std::vector<int> const& acum) {
	for (int i = 0; i < datos.m; i++) {
		...
		if (!marcas[i]) { // esValida
			...
			if (k == datos.n - 1) {
				...
				if (sol.sumaSatis + acum[k+1] > satisMejor) // Estimacion
					repartirRegalos(datos, sol, marcas, k + 1, satisMejor, acum);
			}
			// juguete desmarcado
			marcas[i] = false;
			sol.sumaSatis -= datos.satisfaccion[k][i];
...
bool resuelveCaso() {
	...
	// vector de maximos
	std::vector <int> acum(datos.n);
	for (int i = 0; i < datos.n; i++) {
		int auxMax = datos.satisfaccion[i][0];
		for (int j = 1; j < datos.m; j++)
			if (auxMax < datos.satisfaccion[i][j]) auxMax = datos.satisfaccion[i][j];
		acum[i] = auxMax;
	}
	...
	// Calcular una de las posibles soluciones
	for (int i = 0; i < datos.n; i++) {
		satisMejor += datos.satisfaccion[i][i];
	}

	int k = 0;
	std::vector<bool> marcas(datos.m, false);
	sol.solucion.assign(datos.n, 0);
	repartirRegalos(datos, sol, marcas, k, satisMejor, acum);
	std::cout << satisMejor << "\n";
	return true;
}
